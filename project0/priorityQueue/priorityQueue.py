...
def PQSort(elements) -> list:
    ...
    try:
        # Making sure the argument is an iterable
        iter(elements)
    except TypeError:
        return []
    ...
    pq = PriorityQueue()
    for e in elements:
        # the value of each element is also the priority of it in the queue
        pq.pushAllowDuplicates(e, e)

    # Creating the list that will be returned
    sorted = []
    while not pq.isEmpty():
        # elements are popped in increasing priority and as a result, in increasing value.    
        sorted.append(pq.pop())
    return sorted

def priorityQueueTest():
    """ Test routine for PriorityQueue. """
    ...
    pq.push('New', 1)
    pq.pop()
    print('Empty Queue: %s' % pq.isEmpty())
    if 'New' in pq:
        print("OK")
    if 'Old' in pq:
        print("NOT OK")
    print( PQSort([4, 2, 3, 1, 6, 10, 6, 9]) )
